...
// --- EXTREME DEBUGGING START (keeping for initial verification, can remove later) ---
console.log(`[STARTUP DEBUG] Raw FIREBASE_SERVICE_ACCOUNT_KEY from ENV: '${process.env.FIREBASE_SERVICE_ACCOUNT_KEY}'`);
console.log(`[STARTUP DEBUG] Type of FIREBASE_SERVICE_ACCOUNT_KEY from ENV: ${typeof process.env.FIREBASE_SERVICE_ACCOUNT_KEY}`);
...
// Initialize Firebase Admin SDK by reading the key from a file
try {
  const serviceAccountKeyPath = path.join(__dirname, 'firebase-key.json'); // Path to your new JSON file
  const serviceAccountKeyString = fs.readFileSync(serviceAccountKeyPath, 'utf8');

  console.log("[FIREBASE INIT] Attempting to parse Firebase Service Account Key from file.");
  console.log(`[FIREBASE INIT] Key string length from file: ${serviceAccountKeyString.length}`);
  console.log(`[FIREBASE INIT] Snippet around pos 0-20 from file: '${serviceAccountKeyString.substring(0, 20)}'`);
  console.log(`[FIREBASE INIT] Snippet around pos 0-10 (raw chars) from file: ${Array.from(serviceAccountKeyString.substring(0, 10)).map(c => `U+${c.charCodeAt(0).toString(16).padStart(4, '0')}`).join(' ')}`);
  ...
  console.error("[FIREBASE INIT ERROR] Failed to initialize Firebase Admin SDK. Error reading/parsing firebase-key.json:", e);
  process.exit(1); // Exit if critical component fails
}
