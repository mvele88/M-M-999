...
/**
 * Endpoint to check the *actual* BTC balance using Blockchain.com API (for xPub).
 * This now uses EXODUS_BTC_XPUB environment variable.
...
app.get('/btc-payout-balance', async (req, res) => {
  const exodusBtcXpub = process.env.EXODUS_BTC_XPUB;

  // NEW DEBUG LINE ADDED HERE:
  console.log(`[DEBUG_XPUB] EXODUS_BTC_XPUB from process.env: '${exodusBtcXpub}' (length: ${exodusBtcXpub ? exodusBtcXpub.length : 'N/A'})`);


  if (!exodusBtcXpub) {
    console.error('[BTC Balance Error] Missing EXODUS_BTC_XPUB.');
    return res.status(500).json({ error: 'Missing EXODUS_BTC_XPUB for BTC balance check.' });
  }

  try {
    // Blockchain.com API for xpub balance
    const blockchainComUrl = `https://blockchain.info/balance?active=${exodusBtcXpub}`;
    console.log(`[DEBUG] Constructed Blockchain.com URL: ${blockchainComUrl}`);

    const response = await axios.get(blockchainComUrl);

    console.log('[DEBUG] Raw Blockchain.com API Response Status:', response.status);
    console.log('[DEBUG] Raw Blockchain.com API Response Data:', JSON.stringify(response.data, null, 2));

    // Expected response structure from Blockchain.com for xpub balance
    // Example: {"xpub_address_here": {"final_balance": 12345678, "n_tx": 5, ...}}
    if (response.data && response.data[exodusBtcXpub] && typeof response.data[exodusBtcXpub].final_balance === 'number') {
      const confirmedBalanceSatoshis = response.data[exodusBtcXpub].final_balance;
      const btcBalance = confirmedBalanceSatoshis / 1e8; // Convert satoshis to BTC
      res.json({ balance: btcBalance.toFixed(8) });
    } else {
      console.error('[BTC Balance Error] Unexpected Blockchain.com response structure:');
      console.error('Full response.data:', JSON.stringify(response.data, null, 2));
      res.status(500).json({ error: 'Failed to fetch actual BTC payout balance. Unexpected Blockchain.com response structure or invalid xPub.' });
    }

  } catch (error) {
    console.error('Error fetching actual BTC payout balance:', error.message);
    if (error.response) {
      console.error('Blockchain.com API Error Response Status:', error.response.status);
      console.error('Blockchain.com API Error Response Headers:', JSON.stringify(error.response.headers, null, 2));
      console.error('Blockchain.com API Error Response Data (from error.response):', JSON.stringify(error.response.data, null, 2));
      if (error.response.data && error.response.data.error) {
        console.error('Blockchain.com specific error message:', error.response.data.error);
      }
    }
    res.status(500).json({ error: `Failed to fetch actual BTC payout balance. Detail: ${error.message}` });
  }
});
