...
app.get('/btc-payout-balance', async (req, res) => {
  ...
  try {
    ...
    console.log(`[DEBUG] Constructed Blockonomics URL: ${blockonomicsBalanceUrl}`); // Log the URL
    ...
    if (error.response) {
      ...
      // Add a check for specific Blockonomics error messages if they send them
...
app.post('/process-monthly-data', async (req, res) => {
  ...
  try {
    ...
    profitsSnapshot.forEach(doc => {
      totalSolProfitThisMonth += doc.data().solAmount;
    });

    console.log(`Processing month ${targetMonth}. Total SOL profit: ${totalSolProfitThisMonth.toFixed(4)}`);
