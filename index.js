  ...
  try {
    if (!admin.apps.length) { // Prevent re-initialization in hot-reloading environments
      admin.initializeApp({
        ...
        // databaseURL: `https://${JSON.parse(process.env.FIREBASE_SERVICE_ACCOUNT_KEY).project_id}.firebaseio.com` // Optional
      });
    }
    db = admin.firestore();
    ...
    process.exit(1); // Exit if this critical component fails
  }
} else {
